...
2. **DSP Master installed for that IDE.** DSP Master depends on **MtxVec**; the DSP
   Master installer includes it. The install provides the generated C++ headers
   (`SignalTools.hpp`, `MtxVec.hpp`, …), the package import libraries, the runtime
   DLLs, and adds the required search paths. These samples **link against your
   install** — they don't bundle it.
3. **TeeChart** (bundled with RAD Studio) — used by the charting / scope forms.
