...
AnsiString CodePath()
{
	...
	try
	{
		if (reg->OpenKeyReadOnly(DSPRegKey))
			Res = reg->ReadString("DSPBCBDemoPath");
	}
	__finally
	{
		delete reg;
...
//	if (Res =="")
  //		 if (FileExists("BasicDemoW32_CB10.bdsproj")) Res = GetCurrentDir();
...
void __fastcall TfrmMain::PageControl1Change(TObject *Sender)
{
	...
	{
		if (! FileExists(CodePath() + AnsiString("\\CepstrumDemo.cpp"))) // TODO
		{
			TRegistry *reg = new TRegistry();
...
void __fastcall TfrmMain::btnConfigClick(TObject *Sender)
{
	...
	AnsiString tmpDir;
 #endif
	if (SelectDirectory("Folder with DSP Master demo source", "",tmpDir))
	{
		TRegistry *reg = new TRegistry();
		try
		{
			if (reg->OpenKey(DSPRegKey,true)) reg->WriteString("DSPBCBDemoPath",tmpDir);
		}
		__finally
		{
			delete reg;
