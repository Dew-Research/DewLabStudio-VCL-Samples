	...
	: TBasicForm3(Owner)
{
		...
		RichEdit1->Lines->Add("New features in version 6.3.0:");
		RichEdit1->Lines->Add("");
		RichEdit1->Lines->Add("General:");
		RichEdit1->Lines->Add("");
		RichEdit1->Paragraph->Numbering = nsBullet;
		RichEdit1->Paragraph->FirstIndent = 20;
		RichEdit1->Lines->Add("	Updated for Rad Studio Athens 12.2 and MtxVec v6.3.0.");
		RichEdit1->Lines->Add("	SignalUtils.CZT performance has been improved by a factor of 7x.");
		RichEdit1->Lines->Add("	Added SignalUtils.CZT overload with support for State variable, which gives additional 2.5x speed up allowing for a total of 17.5x faster execution.");
		RichEdit1->Lines->Add("	Added SignalUtils.MusicalNotePitch and its vectorized variant SignalUtils.MusicalNotePitchInit for pitch (note) detection.");
		RichEdit1->Lines->Add("	Enabled block vectorization for SignalUtils.GoertzN algorithm, which leads to additional 5-10x faster execution for large frequency spectrums. When multi-threaded it is expected to achieve linear scaling with thread-count.");
		RichEdit1->Lines->Add("	New implementation of SignalUtils.GoertzN algorithm, which is 3x, if at least 10 and 10x faster, if at least 100 frequencies are evaluated concurrently.");
		RichEdit1->Lines->Add("	Improved accuracy of Bonzanigo Phase correction, especially for large frequency spectrums. Bonzanigo is used by the Goertzal algorithm.");
		RichEdit1->Paragraph->Numbering = nsNone;
		RichEdit1->Paragraph->FirstIndent = 0;
		RichEdit1->Lines->Add("");
		RichEdit1->Lines->Add("TSpectrumAnalyzer improvements:");
		RichEdit1->Lines->Add("");
		RichEdit1->Paragraph->Numbering = nsBullet;
		RichEdit1->Paragraph->FirstIndent = 20;
		RichEdit1->Lines->Add("	Enabled the TSpectrumAnalyzer to use the new faster CZT algorithm.");
		RichEdit1->Lines->Add("	Enabled the TSpectrumAnalyzer numerical peak interpolation to work also while CZT is the selected spectral Method.");
		RichEdit1->Lines->Add("	Vectorized all peak searching and peak processing. Multiple overloads were added that work with vectors. Performance improvements are noticable when working with multiple peaks.");
		RichEdit1->Lines->Add("	Numerical Peak interpolation is now 16x faster for signals which are zero padded 16x. It is now invariant to zero padding factor.");
		RichEdit1->Lines->Add("	Numerical Peak interpolation algorithm has been vectorized delivering at least a 6x speed up. The implementation is our first optimization algorithm that runs multiple jobs in parallel using only vectorization on one CPU core. (Without threading).");
		RichEdit1->Lines->Add("	Numerical Peak interpolation algorithm can in given conditions execute also 96x faster: 6x from interpolation improvements and 16x from zero padding.");
		RichEdit1->Lines->Add("	TSpectrumAnalyzer.Peaks.FindLargestPeaks is typcally more than 100x faster when using numerical peak interpolation, which helps all methods that depend on it.");
		RichEdit1->Lines->Add("	Added TSpectrumAnalyzer.Peaks.FindFirstHarmonic. Although generic, it can be used to detect pitch frequency of the sound (music).");
		RichEdit1->Lines->Add("	Added TSpectrumAnalyzer.Peaks.FindFundamentals. Will find all harmonic series in the frequency spectrum and store their fundamentals. The result is sorted in ascending order according to frequency.");
		RichEdit1->Paragraph->Numbering = nsNone;
		RichEdit1->Paragraph->FirstIndent = 0;
		RichEdit1->Lines->Add("");
		RichEdit1->Lines->Add("TSignalCore Audio improvements:");
		RichEdit1->Lines->Add("");
		RichEdit1->Paragraph->Numbering = nsBullet;
		RichEdit1->Paragraph->FirstIndent = 20;
		RichEdit1->Lines->Add("	Expanded TSignalCoreAudio.ChannelMapping capability allowing arbitrary mix of channels for recording and playback. Each TSignalCoreAudio[i] item now has a Channels: TIntegersList property, which complenents the existng Channel: integer property.");
		RichEdit1->Paragraph->Numbering = nsNone;
		RichEdit1->Paragraph->FirstIndent = 0;
		RichEdit1->Lines->Add("");
		...
		RichEdit1->SelAttributes->Color = (TColor) RGB(51,51,153);
		RichEdit1->SelAttributes->Size = 12;
		RichEdit1->Lines->Add("New features in version 6.2.0:");
		RichEdit1->Lines->Add("");
		RichEdit1->Paragraph->Numbering = nsBullet;
		RichEdit1->Paragraph->FirstIndent = 10;
		RichEdit1->Lines->Add("   Updated for Rad Studio Athens 12.0 and MtxVec v6.2.0.");
		RichEdit1->Lines->Add("   Enhanced click/pop protection for the TSignalCoreAudioOut.Paused functionality. ");
		RichEdit1->Lines->Add("   Bug fix for TSignalCoreAudioOut.Paused.");
