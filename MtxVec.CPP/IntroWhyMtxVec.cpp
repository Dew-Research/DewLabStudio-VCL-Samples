...
//  TBasicForm3::FormCreate(Sender);
  ...
  RichEdit1->Lines->Add("Many linear algebra routines are internally multithreaded, including FFT�s and sparse matrix solvers.");
  ...
  RichEdit1->Lines->Add("Although some compilers support native SSE4/AVX2/AVX512 instruction set, the resulting code can never be as optimal as a hand optimized version.");
  ...
  RichEdit1->Lines->Add("C/C++ and FORTRAN based functions were compiled with the latest version�s Intel C/C++ and FORTRAN compilers and with support for all CPU architectures.");
  ...
  RichEdit1->Lines->Add("The advantage of SSE4 and AVX2, AVX512 instruction sets can be exploited only with unmanaged code. And only hand optimized code can be truly optimal.");
  ...
  RichEdit1->Lines->Add(str);
  str = "Bypasses .NET garbage collector to manage large vectors and matrices more efficiently. The .NET garbage collector uses a special heap for objects greater ";
  str+= "than 80KB, where the management is more inefficient. Many MtxVec functions provide multithreaded primitives.";
  RichEdit1->Lines->Add(str);
