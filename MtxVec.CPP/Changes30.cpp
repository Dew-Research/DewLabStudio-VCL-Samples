...
__fastcall TfrmChanges30::TfrmChanges30(TComponent* Owner): TBasicForm3(Owner)
{
	...
	RichEdit1->Lines->Add("List of new features in v6.4 (May 2026):");
	RichEdit1->Lines->Add("");
	RichEdit1->Lines->Add("Math387.pas:");
	RichEdit1->Lines->Add("");
	RichEdit1->Paragraph->FirstIndent = 10;
	RichEdit1->Paragraph->Numbering = nsBullet;
	RichEdit1->Lines->Add("Math387 scalar math upgraded to use SSE42/AVX2 assembler. 74 functions rewriten, 72.000 lines of new assembler.");
	RichEdit1->Lines->Add("Matches and exceeds the speed of Intel ICX C/C++ math library.");
	RichEdit1->Lines->Add("Sub-1 ULP accuracy across the full input range for every function, every variant. Bit-exact identical results across Intel and AMD.");
	RichEdit1->Lines->Add("Added single-precision variants to all functions that also run up to 2x faster than their double precision counterparts.");
	RichEdit1->Lines->Add("Automatic AVX2+FMA fast path on modern CPUs, SSE4.2 baseline everywhere else. XE6 required for SSE4.2, Rad Studio 11.0+ for AVX2.");
	RichEdit1->Lines->Add("Roughly 7x faster on Win32 and 6x on Win64 in average across all 74 functions.");
	RichEdit1->Lines->Add("Payne Hanek large argument reduction for all trigonometric functions allows accurate result also for huge arguments.");
	...
	RichEdit1->SelAttributes->Color = (TColor) RGB(51,104,196);
	RichEdit1->SelAttributes->Size = 11;
	RichEdit1->Lines->Add("List of new features in v6.3.7 (August 2025):");
	RichEdit1->Lines->Add("");
	RichEdit1->Lines->Add("Core product:");
	RichEdit1->Lines->Add("");
	RichEdit1->Paragraph->FirstIndent = 10;
	RichEdit1->Paragraph->Numbering = nsBullet;
	RichEdit1->Lines->Add("Separated threading library (MtxForLoop.pas) from dependancy upon MtxVec. Moved MtxVec dependant functions to MtxVec unit. This allows the use of the threading library also in units which do not depend upon MtxVec (MtxVec Core, Lapack, Blas, etc...).");
	RichEdit1->Paragraph->FirstIndent = 0;
	RichEdit1->Paragraph->Numbering = nsNone;
	RichEdit1->Lines->Add("");
	...
	RichEdit1->Paragraph->FirstIndent = 10;
	RichEdit1->Paragraph->Numbering = nsBullet;
	RichEdit1->Lines->Add("Debugger Tooltip visualizer launch button sometimes caused a lock-up, because it triggered multiple button press events.");
	RichEdit1->Lines->Add("Improved the automatic naming of debugger mapped variables for cases, where chars like [,] and dot were in the expression.");
	RichEdit1->Lines->Add("Significantly improved automatic column sizing for TMtxVecGrid, which is also used by the debugger visualizer.");
	RichEdit1->Paragraph->FirstIndent = 0;
	RichEdit1->Paragraph->Numbering = nsNone;
	RichEdit1->Lines->Add("");
	...
	RichEdit1->SelAttributes->Color = (TColor) RGB(51,104,196);
	RichEdit1->SelAttributes->Size = 11;
	RichEdit1->Lines->Add("List of new features in v6.3.6 (June 2025):");
	RichEdit1->Lines->Add("");
	RichEdit1->Lines->Add("Core product:");
	RichEdit1->Lines->Add("");
	RichEdit1->Paragraph->FirstIndent = 10;
	RichEdit1->Paragraph->Numbering = nsBullet;
	RichEdit1->Lines->Add("Added ColumnWidth parameter to TVec/TMtx/TVecInt/TMtxInt method ValuesToStrings. Usefull for fixed font width text table formating.");
	RichEdit1->Lines->Add("Renamed TMtx.Mul overload for arrays of TMtx type to TMtx.MulArray to avoid a compiler bug.");
	RichEdit1->Paragraph->FirstIndent = 0;
	RichEdit1->Paragraph->Numbering = nsNone;
	RichEdit1->Lines->Add("");
	RichEdit1->Lines->Add("Special functions:");
	RichEdit1->Lines->Add("");
	RichEdit1->Paragraph->FirstIndent = 10;
	RichEdit1->Paragraph->Numbering = nsBullet;
	RichEdit1->Lines->Add("Bug fix for Bessel J function from SpecialFuncs.pas unit. Introduced due to upgraded compiler.");
	RichEdit1->Lines->Add("Expanded Bessel functions with computations on array with integer step between consecutive elements.");
	RichEdit1->Paragraph->FirstIndent = 0;
	RichEdit1->Paragraph->Numbering = nsNone;
	RichEdit1->Lines->Add("");
	...
	RichEdit1->Paragraph->FirstIndent = 10;
	RichEdit1->Paragraph->Numbering = nsBullet;
	RichEdit1->Lines->Add("Bug fix for Spline1D, which did not work correctly for X other than 0,1,2,3..");
	RichEdit1->Lines->Add("Bug fix for TPiecePoly.Evaluate(X, Y), which did not work for scalar overload, if vectorized overload was not called before.");
	RichEdit1->Paragraph->FirstIndent = 0;
	RichEdit1->Paragraph->Numbering = nsNone;
	RichEdit1->Lines->Add("");
	...
	RichEdit1->SelAttributes->Color = (TColor) RGB(51,104,196);
	RichEdit1->SelAttributes->Size = 11;
	RichEdit1->Lines->Add("List of new features in v6.3.5 (April 2025):");
	RichEdit1->Lines->Add("");
	RichEdit1->Lines->Add("Core product:");
	RichEdit1->Lines->Add("");
	RichEdit1->Paragraph->FirstIndent = 10;
	RichEdit1->Paragraph->Numbering = nsBullet;
	RichEdit1->Lines->Add("Updated installers to install packages and search paths also for the Modern C++ Compiler and for the 64bit IDE preview.");
	RichEdit1->Lines->Add("Bug fix for MtxExpr.h which resulted in the calling of ud2 instruction for Modern C++ compiler of the C++Builder");
	RichEdit1->Lines->Add("Bug Fix for TMtx.MulElem(a: TMtxVec), which was calling the wrong overload.");
	RichEdit1->Lines->Add("Catastrophic cancellation mitigation for numerical GradHess routine for cases, where gradient is near zero. The routine is used by the Optimization methods.");
	RichEdit1->Lines->Add("Added TVec.Mask overload");
	RichEdit1->Lines->Add("Added support for saving and loading the header row to the TVec/TMtx Caption property when saving/loading .csv files with LoadFromFile/SaveToFile method.");
	RichEdit1->Paragraph->FirstIndent = 0;
	RichEdit1->Paragraph->Numbering = nsNone;
	RichEdit1->Lines->Add("");
	...
	RichEdit1->Paragraph->FirstIndent = 10;
	RichEdit1->Paragraph->Numbering = nsBullet;
	RichEdit1->Lines->Add("Added three new NormalPDF, NormalCDF, NormalCDFTwoTail vectorized overloads to probabilities. pas");
	RichEdit1->Lines->Add("Faster vectorized TriangularCDF, TriangularCDFInv, TriangularPDF");
	RichEdit1->Lines->Add("Faster vectorized InverseGaussianPDF and InverseGaussianCDF");
	RichEdit1->Paragraph->FirstIndent = 0;
	RichEdit1->Paragraph->Numbering = nsNone;
	RichEdit1->Lines->Add("");
