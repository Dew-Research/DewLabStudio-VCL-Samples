# CLAUDE.md — Stats Master for C++Builder (examples)

Guidance for building, running, and extending these **Stats Master** (Dew Stats)
C++Builder samples in a fresh checkout. Stats Master is a statistics library
(descriptive stats, hypothesis tests, regression, time series, quality control,
distributions) built on **MtxVec**; the flagship sample is **`StatDemoW32`**, a VCL
app with one form / wizard per area.

There is **one `StatDemoW32_BC<NN>.cbproj` per RAD Studio version**; they all share
...
`_BC17→10.0  _BC18→11.0  _BC19→12.0  _BC20→14.0 (XE6)  _BC21→15.0  _BC22→16.0
_BC23→17.0  _BC24→18.0  _BC25→19.0  _BC26→20.0  _BC27→21.0  _BC28→22.0
_BC29→23.0 (12 Athens)  _BC37→37.0 (latest)`

## Build & run

**In the IDE:** open `StatDemoW32_BC<NN>.cbproj`, choose a **Platform** (`Win32`,
`Win64`, or `Win64x`) and **Config** (`Debug`/`Release`), then Build and Run.

**Command line** — match `rsvars.bat` to the project suffix (`_BC37` ↔ Studio 37):

```bat
call "C:\Program Files (x86)\Embarcadero\Studio\37.0\bin\rsvars.bat"
msbuild StatDemoW32_BC37.cbproj /t:Build /p:Config=Debug /p:Platform=Win64
```
...
(`bcc64`), `Win64x` (`bcc64x` modern Clang; RAD Studio 12 / `_BC29` and newer).

## Running

The built `.exe` needs the **runtime DLLs on `PATH`** (Stats Master + the MtxVec
...
- **`Fatal: Unable to open file 'UNIT.OBJ'`** or **missing `StatTools.hpp` /
  `MtxVec.hpp`** at build: Stats Master isn't installed for *this* IDE, or its
  packages weren't built with C++Builder output. Reinstall for this RAD Studio
...
## Writing your own Stats Master / MtxVec C++ code
...
#include "StatTools.hpp"         // statistics components
#include "StatChar.hpp"          // descriptive stats / distributions
#include "StatTimeSerAnalysis.hpp" // time-series (as needed)
```
...
`AllPackageLibs` — the Stats Master packages are `StatCore<NN>` (engine),
`StatTools<NN>` (components) and `StatSeries<NN>` (charts), on top of MtxVec's
`MtxCore<NN>` / `MtxTools<NN>` / `MtxTee<NN>`, alongside the standard RAD Studio
(`rtl`, `vcl`, …) and TeeChart (`Tee9<NN>` / `TeePro9<NN>` / `TeeUI9<NN>`) packages.
...
1. **In a callback, don't index the bare `->Values[i]` *field* on a vector the
   library hands you** (regression/optimization/fitting callbacks, etc.). On a
   `TVec*`, `Values` is a `DynamicArray<double>` whose C++ `operator[]` is
   **bounds-checked in Debug *and* Release**, and the library may pass a **sub-range
   view** whose internal length is not a real array header → a false "range"
   exception, and the program disappears. Instead **typecast to `Mtxvec::TVector*` /
   `Mtxvec::TMatrix*`** (thin subclasses, declared in `MtxVec.h`, whose `Values[]` is
   plain inline pointer math) and keep the natural syntax:
